...
from app.schemas.forecast import ForecastPoint, ForecastOut, ProductChannelForecast, SeasonalityFactor
...
def get_forecast_data(tenant_code: str, start: str, end: str) -> ForecastOut:
    """Genera pronóstico de ventas con análisis detallado."""
    db = SessionLocal()
    ...
    try:
        ...
        while current_date <= end_date:
            ...
            if ma_values:
                predicted_value = predicted_value * 0.6 + ma_values[-1] * 0.4
            
            day_name = current_date.strftime("%A")
            seasonality_factor = seasonality.get(day_name, 1/7)
            predicted_value = predicted_value * (seasonality_factor * 7)
            
            predicted_value = max(predicted_value, avg_daily_sales * 0.5)
